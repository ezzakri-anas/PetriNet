...
class PlaceTest {
	...
	void testPlace() {
		...
		assertEquals(p2.getNbreJetons(), 0);// Le nombre de jetons devrait être ramené à 0 dans ce cas.
	...
	void testGetNbreJetons() {
		...
		assertEquals(p.getNbreJetons(), 10);// Vérifie que le nombre de jetons est bien 10.
	...
	void testSetNbreJetons() {
		Place p = new Place(10);
		p.setNbreJetons(7);
		assertEquals(p.getNbreJetons(), 7);// Vérifie si le nombre de jetons est correctement mis à jour.
		...
		p2.setNbreJetons(-1);
		assertEquals(p2.getNbreJetons(), 0);// Le nombre de jetons doit être ramené à 0.
	...
	void testRmNbreJetons() {
		Place p = new Place(10);
		p.rmNbreJetons(7);
		assertEquals(p.getNbreJetons(), 10 - 7);// Vérifie que le nombre de jetons est réduit correctement.
		...
		Place p2 = new Place(10);
		p2.rmNbreJetons(11);
		assertEquals(p2.getNbreJetons(), 0);// Tous les jetons doivent être retirés dans ce cas.
